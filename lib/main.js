...
function padjust( pvals, method, comparisons ) {
	if ( !isProbabilityArray( pvals ) ) {
		throw new TypeError( format( '1Ll9x,GK', pvals ) );
	}
	if ( !isString( method ) ) {
		throw new TypeError( format( '1Ll39,Ey', method ) );
	}
	if ( arguments.length > 2 ) {
		if ( !isInteger( comparisons ) ) {
			throw new TypeError( format( '1Ll2z,GL', comparisons ) );
		}
		if ( comparisons < pvals.length ) {
			throw new RangeError( format( '1LlGM', comparisons ) );
		}
	}
	...
	default:
		throw new Error( format( '1LlGN,Mn', METHODS.join( '", "' ), method ) );
	}
}
