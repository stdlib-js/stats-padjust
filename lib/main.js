...
function padjust( pvals, method, comparisons ) {
	if ( !isProbabilityArray( pvals ) ) {
		throw new TypeError( format( '1Ll9x', pvals ) );
	}
	if ( !isString( method ) ) {
		throw new TypeError( format( '1Ll39', method ) );
	}
	if ( arguments.length > 2 ) {
		if ( !isInteger( comparisons ) ) {
			throw new TypeError( format( '1Ll2z', comparisons ) );
		}
		if ( comparisons < pvals.length ) {
			throw new RangeError( format( '1LlDP', comparisons ) );
		}
	}
	...
	default:
		throw new Error( format( '1LlDQ', METHODS.join( '", "' ), method ) );
	}
}
